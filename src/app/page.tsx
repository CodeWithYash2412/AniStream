...
export default function Home() {
  const trendingAnime = [...mockAnime].sort((a, b) => b.rating - a.rating);
  const topAiring = [...mockAnime].filter(a => a.status === 'Airing').slice(0, 10);
  const popular = [...mockAnime].sort(() => 0.5 - Math.random()).slice(0, 10);
  const newlyAdded = [...mockAnime].sort((a,b) => new Date(b.addedDate).getTime() - new Date(a.addedDate).getTime()).slice(0,10);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="space-y-12 py-12">
        ...
        <AnimeCarousel title="Newly Added" animeList={newlyAdded} />
      </div>
    </div>
  );
}
